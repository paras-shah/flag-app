...
class App extends React.Component {
  ...
  onContinentSelect = (selectedValueIndex) => {
    // get country data
    const countryList =  this.getCountry(selectedValueIndex);
    this.setState({ 
      selectedContinentIndex: selectedValueIndex,
      selectedContinentCountryData: countryList
    });
  }
  ...
  //
  getCountry = (continentIndex) => {
    const selectedContinentData = flagData[continentIndex];

    return selectedContinentData.map((value) => {
      return value["continent"];
    });
  }
