        ...
        if (possibleValues.length >= 0) {
            if (multiSelect) {
                return possibleValues.map((value, index) => {
                    ...
                    const isSelected = selectedValue.indexOf(value)<-1?' defaultChecked={true} ':'';
                    
                    return (<li key={getIndex} >
                        <input defaultChecked={false} onClick={(e) => { this.onSelect(value, e) }} type="checkbox" name="country" aria-label={value} id={`country${index}`} />
