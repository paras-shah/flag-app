...
class Search extends React.Component {
    ...
    onSelect = (selectedValue) => {
        ...
        if (multiSelect) {
            ...
            this.setState({ selectedValue: selectedValueIndex });
    ...
    checkboxClick = (event, checkBoxValue) => {
        const { selectedValue, allValues } = this.state;
        ...
        showOptions = () => {
            const { possibleValues, multiSelect } = this.state;
            if (possibleValues.length >= 0) {
                if (multiSelect) {
                    return possibleValues.map((value, index) => {
                        return (<li key={index} onClick={(e) => { this.onSelect(value) }}>
                            <input type="checkbox" name="country" aria-label={value} id={`country${index}`} />
