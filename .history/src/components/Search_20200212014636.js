...
class Search extends React.Component {
    ...
    onSelect = (selectedValue) => {
        const { allValues, multiSelect } = this.state;
            ...
            if (multiSelect) {
                return possibleValues.map((value, index) => {
                    return (<li key={index} onClick={() => { this.onSelect(value) }}>
                        <input type="checkbox" name="country" aria-label={value} aria-required="true" id={`country${index}`} onClick={() => {
                            this.checkboxClick(value)
                        }} /><label forHtml={`country${index}`}>{value}</label>
                    </li>);
                })
            }
            else {
