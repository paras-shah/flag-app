...
class Search extends React.Component {
    ...
    static getDerivedStateFromProps(props) { 
        return { 
        editingQuark: checked:props.checked 
    }; }
        ...
        if (possibleValues.length >= 0) {
            if (multiSelect) {
                return possibleValues.map((value, index) => {
                    ...
                    const isSelected = selectedValue.indexOf(value) > -1 ? true : false;

                    return (<li key={getIndex} >
                        <input defaultChecked={isSelected} onClick={(e) => { this.onSelect(value, e) }} type="checkbox" name="country" aria-label={value} id={`country${index}`} />
